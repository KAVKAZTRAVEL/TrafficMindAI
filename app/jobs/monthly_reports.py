...
@celery_app.task(name="app.jobs.monthly_reports.generate_monthly_reports")
def generate_monthly_reports() -> int:
...
async def _generate() -> int:
    ...
    async with SessionLocal() as session:
        ...
        for website in websites:
            sources = await aggregate_sources(session, website.id, "monthly")
            session.add(Report(website_id=website.id, period="monthly", ai_summary=explain_sources(sources)))
            count += 1
