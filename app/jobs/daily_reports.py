...
@celery_app.task(name="app.jobs.daily_reports.generate_daily_reports")
...
async def _generate() -> int:
    ...
    async with SessionLocal() as session:
        ...
        for website in websites:
            sources = await aggregate_sources(session, website.id, "daily")
            session.add(Report(website_id=website.id, period="daily", ai_summary=explain_sources(sources)))
            count += 1
